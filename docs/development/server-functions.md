...
7. If a settings change affects services created during application startup, call `resetApp()` after saving it. Event-stream clients reconnect automatically.
...
For mutations, pass `useServerFn(fn)` as the `mutationFn` in `useMutation`. The global `/api/events` event stream invalidates all queries after a change, so do not add function-specific invalidation.
