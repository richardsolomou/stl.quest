...
| `DATABASE_URL`                                                        | —         | Turso or libSQL URL (`libsql://`, `https://`, or `wss://`). When unset, STL Quest uses SQLite under `DATA_DIR`.                 |
| `DATABASE_AUTH_TOKEN`                                                 | —         | Authentication token for `DATABASE_URL`.                                                                                        |
...
Keep `/data` on a local filesystem. SQLite WAL databases should not be placed on NFS, SMB, or CIFS. A remote libSQL database does not remove the need for `/data`, which still holds upload staging and the generated integration encryption key.
...
Automatic migrations create a SQLite snapshot under `/data/backups` immediately before changing the schema. Remote libSQL deployments rely on their database provider's backup and point-in-time recovery instead. Database backups do not include local or cloud model storage, upload staging, or an environment-provided encryption key.
...
The command supports local SQLite only and does not copy model storage. For remote libSQL, use the provider's backup tooling. If `INTEGRATIONS_ENCRYPTION_KEY` supplies the key, store that secret in your backup system separately because there is no key file for the command to copy.
