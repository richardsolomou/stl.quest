...
| `STLQUEST_TRUSTED_PROXY_CIDRS`                                                       | private ranges | Space-separated CIDRs for the immediate HTTPS reverse proxy. Narrow this to the proxy network when the container port is reachable by other private-network clients. |
...
The bundled proxy trusts forwarded headers from private address ranges by default so zero-configuration Docker proxy networks work. Set `STLQUEST_TRUSTED_PROXY_CIDRS` to the immediate proxy network, such as `172.18.0.0/16`, when other clients can reach port 3000 over a private network.
