...
`DATA_DIR` (default `/data`) selects app data, `PRINTS_DIR` (default `/prints`) selects default local storage, `ASSET_JOB_CONCURRENCY` (default `1`) controls simultaneous visual and orientation jobs without an application-level maximum, and `METRICS_TOKEN` optionally protects Prometheus metrics. Each asset worker is primarily single-threaded and normally occupies one CPU core, so concurrency `1` is recommended on TrueNAS; increasing it runs more models in parallel rather than giving one model more CPU. Product settings — storage backend, board visibility, telemetry, authentication, and email delivery — live in the admin-only **Settings** page and persist in `/data/printhub.sqlite`. Integration secrets are encrypted with `/data/integration-secrets.key` unless `INTEGRATIONS_ENCRYPTION_KEY` is supplied.
...
- `SIGINT` and `SIGTERM` stop new singleton use, close SSE streams, finish only currently running asset stages, and leave queued durable work for the next process before closing SQLite cleanly.
