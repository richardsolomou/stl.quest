...
ALTER TABLE "managed_storage_usage" ADD CONSTRAINT "managed_storage_usage_workspace_id_organization_id_fk" FOREIGN KEY ("workspace_id") REFERENCES "public"."organization"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
CREATE INDEX "managed_storage_entitlements_owner" ON "managed_storage_entitlements" USING btree ("owner_id");