...
test('requesters own queue priority while admins move work between stages', async ({ page, browser }) => {
  ...
  await expect(requesterPage.getByText('Inspect a print complete')).toBeVisible()
  await expect(requesterPage.getByText('+10 XP earned')).toBeVisible()
  await expect(requesterPage.getByRole('button', { name: 'View onboarding' })).toBeVisible()
  ...
  await expect(requesterPage.getByText('STL Quest complete!')).toBeVisible()
