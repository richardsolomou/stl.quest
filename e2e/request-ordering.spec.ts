...
test('requesters own queue priority while admins move work between stages', async ({ page, browser }) => {
  ...
  const requesterTour = requesterPage.getByRole('alertdialog', { name: 'STL Quest' })
  ...
  await requesterPage.getByRole('button', { name: 'STL Quest, 2 of 3 resolved, 30 XP' }).click()
  ...
  await requesterPage.getByRole('button', { name: 'STL Quest, 3 of 3 resolved, 40 XP' }).click()
