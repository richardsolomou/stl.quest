...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  await expect(page.getByLabel('Printer name').first()).toHaveValue('Elegoo Mars 2')
  await expect(page.getByLabel(/Usable width|Usable depth|Usable height/)).toHaveCount(0)
  await fillPrinter(page.getByText('Printer name').locator('..').locator('..'), { name: 'Resin Station', printType: 'Resin' })
  ...
  await expect(page.getByRole('combobox', { name: 'Printer', exact: true })).toContainText('Resin Station (automatic)')
  ...
  await expect(requestCard(page, 'first-model')).toContainText('Resin Station')
