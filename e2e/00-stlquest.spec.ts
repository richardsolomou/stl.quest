...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  const tour = page.getByRole('region', { name: 'Getting started' })
  await expect(tour.getByRole('heading', { name: 'Learn by doing' })).toBeVisible()
  await expect(tour.getByText('0 of 5 complete')).toBeVisible()
  await expect(tour.getByText('Drag STL files anywhere onto the board')).toBeVisible()
  await screenshot(page, 'product-tour-board')
  await expect(page.getByRole('button', { name: 'Add a print' })).toHaveAttribute('data-onboarding-active', 'true')
  await tour.getByRole('button', { name: 'Choose files' }).click()
  await expect(page.getByRole('dialog', { name: 'Add prints' })).toBeVisible()
  await page.getByRole('dialog', { name: 'Add prints' }).getByRole('button', { name: 'Close' }).click()
  ...
  await page.getByRole('button', { name: 'Open getting started' }).click()
  await expect(tour.getByText('Know where models are stored')).toBeVisible()
  await tour.getByRole('button', { name: 'Mark as reviewed' }).click()
  await expect(tour).toHaveCount(0)
