...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  await expect(longTagDot).toContainText(`${longTag} · Select all`)
  expect((await longTagDot.locator('span').first().boundingBox())!.width).toBeLessThanOrEqual(164)
  ...
  await expect(adjacentTagDot).toContainText('Plate 14 · Select all')
