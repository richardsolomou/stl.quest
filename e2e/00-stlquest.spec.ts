...
test('manages a fair print queue and assigns work to printers', async ({ browser, page }) => {
  ...
  const tour = page.getByRole('alertdialog', { name: 'Getting started' })
  await expect(tour.getByRole('heading', { name: 'Add your first print' })).toBeVisible()
  await expect(tour.getByText('Getting started · 1 of 3')).toBeVisible()
  ...
  await tour.getByRole('button', { name: 'Remind me tomorrow' }).click()
  await expect(tour).toHaveCount(0)
  await page.getByRole('button', { name: 'Open account menu' }).click()
  await page.getByRole('button', { name: 'Restart onboarding' }).click()
  await expect(tour.getByRole('heading', { name: 'Add your first print' })).toBeVisible()
  await tour.getByRole('button', { name: 'Skip this tip' }).click()
  await expect(tour.getByRole('heading', { name: 'Choose how the queue is sorted' })).toBeVisible()
  await tour.getByRole('button', { name: 'Skip this tip' }).click()
  await expect(tour.getByRole('heading', { name: 'Focus the board with filters' })).toBeVisible()
  await tour.getByRole('button', { name: 'Skip this tip' }).click()
  await expect(tour).toHaveCount(0)
  await page.reload()
  await expect(tour).toHaveCount(0)
  const otherBrowser = await browser.newPage()
  await otherBrowser.goto('/')
  await otherBrowser.getByLabel('Email').fill(email)
  await otherBrowser.getByLabel('Password').fill(password)
  await otherBrowser.getByRole('button', { name: 'Sign in' }).click()
  await expect(otherBrowser.getByRole('heading', { name: 'Your production queue is ready' })).toBeVisible()
  await expect(otherBrowser.getByRole('alertdialog', { name: 'Getting started' })).toHaveCount(0)
  ...
  await expect(tour.getByRole('heading', { name: 'Know where models are stored' })).toBeVisible()
  await tour.getByRole('button', { name: 'Skip this tip' }).click()
  await expect(tour).toHaveCount(0)
