...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  await requestCard(page, 'bulk-move-single-a').click({ button: 'right' })
  await expect(page.getByRole('menuitem', { name: 'Create batch' })).toBeVisible()
  await screenshot(page, 'batch-create-context-menu')
  await page.getByRole('menuitem', { name: 'Create batch' }).click()
  ...
  const renameBatch = page.getByRole('dialog', { name: 'Rename print batch' })
  await renameBatch.getByLabel('Batch name').fill('Dragon production plate')
  await renameBatch.getByRole('button', { name: 'Rename batch' }).click()
  const renamedBatch = page.getByRole('region', { name: 'Batch Dragon production plate' })
  ...
  await expect(deleteBatch).toContainText('The prints will stay in this stage and become unbatched.')
  await deleteBatch.getByRole('button', { name: 'Delete batch' }).click()
