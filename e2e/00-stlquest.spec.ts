...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Save storage' }).click()
  const migrationReview = page.getByRole('alertdialog', { name: 'Start storage migration?' })
  await expect(migrationReview.getByText('Destination connected and writable')).toBeVisible()
  ...
  await page.getByRole('button', { name: 'Save storage' }).click()
  const populatedDestinationReview = page.getByRole('alertdialog', { name: 'Start storage migration?' })
  await expect(populatedDestinationReview.getByText('Selected folder contents')).toBeVisible()
