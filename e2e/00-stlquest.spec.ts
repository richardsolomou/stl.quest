...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  await expect(page.locator('[data-status="todo"] button.card').filter({ hasText: 'print-again' })).toContainText('×4')
  await expect(page.locator('[data-status="in_progress"] button.card').filter({ hasText: 'print-again' })).toContainText('×1')
