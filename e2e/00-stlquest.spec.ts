...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  await expect(page.getByText('Printers saved', { exact: true })).toBeVisible()
  await page.goto('/')
  ...
  await longPress(requestCard(page, 'bulk-move-single-a'))
  await page.getByRole('menuitem', { name: 'Remove from selection' }).click()
  await page.locator('[data-status="todo"]').getByRole('button', { name: 'Select', exact: true }).click()
  ...
  await expect(page.getByText('Printer archived', { exact: true })).toBeVisible()
  ...
  await expect(page.getByText('Printer restored', { exact: true })).toBeVisible()
