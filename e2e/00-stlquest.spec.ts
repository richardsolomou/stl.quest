...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  await expect(page.getByRole('menuitem', { name: 'Mark failed' })).toHaveAttribute('aria-disabled', 'true')
  ...
  await page.getByRole('menuitem', { name: 'Mark failed' }).click()
  await expect(page.locator('[data-status="todo"] button.card').filter({ hasText: 'bulk-move-single-c' })).toBeVisible()
  await expect(page.getByRole('menuitem', { name: 'Mark failed' })).toHaveCount(0)
  ...
  const contextDelete = page.getByRole('alertdialog', { name: 'Delete “context-delete”?' })
  await contextDelete.getByRole('button', { name: 'Delete request' }).click()
  ...
  await expect(batchDelete.getByText('1 instance', { exact: true })).toBeVisible()
  await expect(batchDelete.getByText('bulk-delete-b')).toBeVisible()
  await expect(batchDelete.getByText('2 instances', { exact: true })).toBeVisible()
  ...
  await page.getByRole('button', { name: 'Clear selection' }).click()
  await page.locator('[data-status="todo"]').getByRole('button', { name: 'Select' }).click()
  await requestCard(page, 'bulk-move-single-a').click()
  await page.getByRole('button', { name: 'Move', exact: true }).click()
