...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  await expect(page.getByText('1 of 11 quests resolved · 10 of 150 XP earned')).toBeVisible()
  ...
  await expect(page.getByText('1 of 11 quests resolved · 10 of 150 XP earned')).toBeVisible()
  await page.getByRole('button', { name: 'Add your first print 20 XP' }).click()
  ...
  await expect(uploadDialog.getByLabel('Title')).toBeVisible()
  ...
  await expect(repeatRequest.getByText('Create new requests for 2 selected prints.')).toBeVisible()
  await repeatRequest.getByLabel('Copies').fill('4')
  await screenshot(page, 'print-again-dialog')
  await repeatRequest.getByRole('button', { name: 'Create requests' }).click()
  const repeatedPrint = page.locator('[data-status="todo"] button.card[data-request-name="print-again"]')
  const repeatedSelectedPrint = page
    .locator('[data-status="todo"] button.card[data-request-name="selected-repeat"]')
    .filter({ hasText: '×4' })
  const originalPrint = page.locator('[data-status="in_progress"] button.card[data-request-name="print-again"]')
  await expect(repeatedPrint).toContainText('×4')
  await expect(repeatedSelectedPrint).toContainText('×4')
  ...
  await repeatedSelectedPrint.click({ button: 'right' })
  await page.getByRole('menuitem', { name: 'Delete' }).click()
  await page
    .getByRole('alertdialog', { name: 'Delete 4 copies of “selected-repeat”?' })
    .getByRole('button', { name: 'Delete copies' })
    .click()
  const originalSelectedPrint = page
    .locator('[data-status="todo"] button.card[data-request-name="selected-repeat"]')
    .filter({ hasText: '×1' })
  await originalSelectedPrint.click({ button: 'right' })
  await page.getByRole('menuitem', { name: 'Delete' }).click()
  await page.getByRole('alertdialog', { name: 'Delete 1 copy of “selected-repeat”?' }).getByRole('button', { name: 'Delete copy' }).click()
  ...
  await expect(page.locator('[data-slot="tooltip-content"][data-open]')).toHaveText(`Select all from ${longTag}`)
  const adjacentTagDot = requestCardTag(requestCard(page, 'bulk-move-single-a'), 'Plate 14')
  const adjacentTagBox = await adjacentTagDot.boundingBox()
  expect(adjacentTagBox).not.toBeNull()
  await page.mouse.move(adjacentTagBox!.x + adjacentTagBox!.width / 2, adjacentTagBox!.y + adjacentTagBox!.height / 2)
  await expect(page.locator('[data-slot="tooltip-content"][data-open]')).toHaveText('Select all from Plate 14')
  ...
  await page.getByRole('menuitem', { name: 'Manage tags' }).click()
  const batchTags = page.getByRole('dialog', { name: 'Tag prints' })
  await batchTags.getByLabel('Find or create tags').fill('Batch tag')
  await batchTags.getByLabel('Find or create tags').press('Enter')
  await expect(requestCardTag(requestCard(page, 'bulk-delete-a'), 'Batch tag')).toHaveCount(1)
  await expect(
  ...
  await longPress(requestCard(page, 'bulk-move-single-a'))
  await page.getByRole('menuitem', { name: 'Remove from selection' }).click()
  await page.getByRole('button', { name: 'Select all in Queue' }).click()
  await expect(page.locator('[data-status="todo"] button.card[aria-pressed="true"]')).not.toHaveCount(0)
  await screenshot(page, 'column-click-selection')
  await page.keyboard.press('Escape')
  await expect(page.locator('button.card[aria-pressed="true"]')).toHaveCount(0)
  ...
  await page.getByRole('button', { name: `Archive ${printerName}` }).click()
  const archivePrinterDialog = page.getByRole('alertdialog', { name: `Archive “${printerName}”?` })
  await expect(archivePrinterDialog.getByText('completed prints will continue to show it')).toBeVisible()
  await archivePrinterDialog.getByRole('button', { name: 'Archive printer' }).click()
  await expect(page.locator('[data-slot="settings-page"]')).toHaveAttribute('data-saving', 'true')
  await expect(page.getByText('Archived printers', { exact: true })).toBeVisible()
  await expect(page.locator('[data-slot="settings-page"]')).toHaveAttribute('data-saving', 'false')
  ...
  await upload(page, { name: 'archive-me', printType: 'Resin', buffer: boxStl('archive-me', 10, 10, 10) })
  await requestCard(page, 'archive-me').click({ button: 'right' })
  await expect(page.getByRole('menuitem', { name: 'Archive' })).toBeVisible()
  await page.getByRole('menuitem', { name: 'Move', exact: true }).click()
  const archiveMove = page.getByRole('dialog', { name: 'Move copies' })
  await choose(archiveMove.getByLabel('Destination'), 'Ready')
  await archiveMove.getByRole('button', { name: 'Move', exact: true }).click()
  await expect(archiveMove).toBeHidden()
  await expect(page.locator('[data-status="todo"] button.card').filter({ hasText: 'archive-me' })).toHaveCount(0)
  await expect(page.locator('[data-status="done"] button.card').filter({ hasText: 'archive-me' })).toBeVisible()
  await page.locator('[data-status="done"] button.card').filter({ hasText: 'archive-me' }).click({ button: 'right' })
  await page.getByRole('menuitem', { name: 'Archive' }).click()
  await page.getByRole('navigation', { name: 'Main navigation' }).getByRole('link', { name: 'Archive' }).click()
  const archiveRow = page.locator('.virtual-row').filter({ hasText: 'archive-me' })
  await expect(archiveRow).toBeVisible()
  await expect(page.getByText('Nothing is archived yet')).toHaveCount(0)
  await screenshot(page, 'archive-view')
  await archiveRow.getByRole('button', { name: 'Move back' }).click()
