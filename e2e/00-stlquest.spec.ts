...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  const questButton = page.getByRole('button', { name: 'STL Quest, 0 of 7 resolved, 0 XP' })
  ...
  await expect(page.getByText('0 of 7 quests resolved · 100 XP available')).toBeVisible()
  await screenshot(page, 'product-quest-list')
  await page.getByRole('button', { name: 'Skip Find work with filters' }).click()
  await expect(page.getByRole('button', { name: 'Restore Find work with filters' })).toBeVisible()
  await page.getByRole('button', { name: 'Restore Find work with filters' }).click()
  await expect(page.getByText('0 of 7 quests resolved · 100 XP available')).toBeVisible()
  await page.keyboard.press('Escape')
  const tour = page.getByRole('alertdialog', { name: 'STL Quest' })
