...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  await expect(populatedDestinationReview.getByText('Workspace folder')).toBeVisible()
  await expect(populatedDestinationReview.getByText(/\d+ files · \d+ folders/)).toBeVisible()
  await expect(populatedDestinationReview.getByText('Files outside this folder are not inspected or changed.')).toBeVisible()
  await screenshot(page, 'storage-destination-contents')
  await populatedDestinationReview.getByRole('button', { name: 'Empty destination first' }).click()
