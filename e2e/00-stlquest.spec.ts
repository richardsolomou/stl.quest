...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Connect my Dropbox' })).toBeDisabled()
  await page.getByRole('button', { name: 'Set up the Dropbox app' }).click()
  const cloudAppDialog = page.getByRole('dialog', { name: 'Set up the Dropbox app' })
  await expect(cloudAppDialog.getByText('/api/storage/dropbox/callback')).toBeVisible()
  ...
  const localStorageSwitch = page.locator('[aria-label="Enable local folder storage"]:visible')
  await expect(localStorageSwitch).toBeChecked()
  await localStorageSwitch.click()
  await expect(localStorageSwitch).not.toBeChecked()
  await screenshot(page, 'integration-storage-providers')
  await page.goto('/settings/storage')
  await expect(page.getByRole('button', { name: /A folder on this server/ })).toHaveCount(0)
  await page.goto('/admin/integrations')
  await page.locator('[aria-label="Enable local folder storage"]:visible').click()
  await expect(page.locator('[aria-label="Enable local folder storage"]:visible')).toBeChecked()
