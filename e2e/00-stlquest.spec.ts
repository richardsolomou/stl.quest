...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  await screenshot(page, 'batch-create-context-menu')
  await page.getByRole('menuitem', { name: 'Add to group' }).click()
  const createBatch = page.getByRole('dialog', { name: 'Create print group' })
  await createBatch.getByLabel('Group name').fill('Dragon plate')
  await createBatch.getByRole('button', { name: 'Create group' }).click()
  const preparedBatch = page.getByRole('region', { name: 'Group Dragon plate' })
  ...
  const groupOrder = () =>
    preparedBatch.locator('button.card').evaluateAll((cards) => cards.map((card) => card.getAttribute('data-request-name')))
  await expect.poll(groupOrder).toEqual(['bulk-move-a', 'bulk-move-b', 'bulk-move-single-c'])
  await dragOnto(
    preparedBatch.getByRole('button', { name: /bulk-move-b/ }),
    preparedBatch.getByRole('button', { name: /bulk-move-a/ }),
    undefined,
    0.1,
  )
  ...
  await preparedBatch.click({ button: 'right' })
  await page.getByRole('menuitem', { name: 'Rename' }).click()
  const renameBatch = page.getByRole('dialog', { name: 'Rename print group' })
  await renameBatch.getByLabel('Group name').fill('Dragon production plate')
  await renameBatch.getByRole('button', { name: 'Rename group' }).click()
  const renamedBatch = page.getByRole('region', { name: 'Group Dragon production plate' })
  await expect(renamedBatch).toBeVisible()
  await renamedBatch.click({ button: 'right' })
  await screenshot(page, 'batch-context-menu')
  await page.getByRole('menuitem', { name: 'Delete group' }).click()
  const deleteBatch = page.getByRole('alertdialog', { name: 'Delete “Dragon production plate”?' })
  await expect(deleteBatch).toContainText('The prints will stay in this stage and leave the group.')
  await deleteBatch.getByRole('button', { name: 'Delete group' }).click()
  await expect(renamedBatch).toHaveCount(0)
