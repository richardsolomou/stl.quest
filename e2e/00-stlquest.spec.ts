...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  const createGroup = page.getByRole('dialog', { name: 'Create print group' })
  await createGroup.getByLabel('Group name').fill('Dragon plate')
  await createGroup.getByRole('button', { name: 'Create group' }).click()
