...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  expect((await page.getByRole('heading', { name: 'Choose storage' }).boundingBox())?.y).toBeGreaterThanOrEqual(0)
  await screenshot(page, 'storage-provider-picker')
  const storageFooter = page.getByText('STL Quest does not host or take ownership of your model files.')
  await storageFooter.scrollIntoViewIfNeeded()
  ...
  await defaultGroup.click({ button: 'right' })
  ...
  await upNextGroup.click({ button: 'right' })
  ...
  await preparedGroup.click({ button: 'right' })
  await page.getByRole('menuitem', { name: 'Rename' }).click()
  ...
  await renamedGroup.click({ button: 'right' })
  await screenshot(page, 'group-context-menu')
  await page.getByRole('menuitem', { name: 'Delete group' }).click()
  const deleteGroup = page.getByRole('alertdialog', { name: 'Delete “Dragon production plate”?' })
  await expect(deleteGroup).toContainText('The prints will stay in this stage and leave the group.')
  ...
  await expect(batchDelete).toContainText('2 affected instances')
  ...
  await page.getByRole('button', { name: 'Save storage' }).click()
  const migrationReview = page.getByRole('alertdialog', { name: 'Review storage change' })
  await expect(migrationReview.getByText('Verified')).toBeVisible()
  await expect(migrationReview.getByText(originalStorageRoot, { exact: true })).toBeVisible()
  await migrationReview.getByRole('button', { name: 'Start migration' }).click()
  ...
  await page.getByRole('button', { name: 'Save storage' }).click()
  const populatedDestinationReview = page.getByRole('alertdialog', { name: 'Review storage change' })
  await expect(populatedDestinationReview.getByText('Destination contents')).toBeVisible()
  await expect(populatedDestinationReview.getByText(/\d+ files · \d+ folders/)).toBeVisible()
  await populatedDestinationReview.getByText('Review files and folders').click()
  await expect(populatedDestinationReview.locator('li').first()).toBeVisible()
  await screenshot(page, 'storage-destination-contents')
  await populatedDestinationReview.getByRole('button', { name: 'Replace folder contents' }).click()
  await populatedDestinationReview.getByRole('button', { name: 'Start migration' }).click()
  const clearDestination = page.getByRole('alertdialog', { name: 'Empty the destination?' })
  await expect(clearDestination).toContainText(originalStorageRoot)
  await expect(clearDestination).toContainText('including data from previous STL Quest workspaces')
  await screenshot(page, 'storage-empty-destination-confirmation')
  await clearDestination.getByRole('button', { name: 'Delete contents' }).click()
  await expect(clearDestination).toHaveCount(0)
  ...
  await page.goto('/')
  await expect(page.getByRole('heading', { name: 'Choose storage' })).toHaveCount(0)
