...
test('manages profile details through the protected account surface', async ({ page }) => {
  ...
  const protectedEmailText = protectedEmail.locator('span').last()
  await expect(protectedEmailText).toHaveCSS('filter', /blur\(5px\)/)
  ...
  await expect(page.getByRole('button', { name: 'owner@example.com' }).locator('span').last()).toHaveCSS('filter', 'none')
