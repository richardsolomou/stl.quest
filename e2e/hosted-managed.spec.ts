...
test('shares included storage across three hosted workspaces and enforces the ownership limit', async ({ page }) => {
  ...
  await expect(page.getByText('Your account includes 5 GB')).toBeVisible()
  ...
  await expect(page.getByRole('link', { name: '5.0 GB storage available' })).toBeVisible()
  ...
  await expect(page.getByText('5.0 GB available')).toBeVisible()
