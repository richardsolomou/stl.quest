...
Install Node 24.x and pnpm 11.12.0, then run:
...
`pnpm check` runs the formatter, linter, database migration checks, printer catalog and Unraid metadata checks, production build, type checker, unit tests, and a quick backup command test. The build comes before type checking because it generates `src/routeTree.gen.ts`.

`pnpm test:e2e` builds the production server and runs the end-to-end tests. Install Chromium once with `pnpm test:e2e:install`. For faster reruns against an existing build, use `pnpm test:e2e:run`. Set `PLAYWRIGHT_DEV_SERVER=1` only when debugging against Vite.
...
Test the online backup command with disposable data by running `DATA_DIR=/tmp/stlquest-test pnpm backup --output /tmp/stlquest-backup.sqlite`. `pnpm check:cli` checks the command-line help.
